...
class App extends Component {
  ...
  songEnded = () => {
    const { songs, playState, repeatType, playSong: play } = this.props;
    // No repeat
    if (repeatType === 0) {
      URL.revokeObjectURL(songs[playState.songId]);
  ...
  render() {
    const {
      ...
      snackMsg,
      installEvent,
      addToHomeScreenUIVisible,
      hideSnackAction
    } = this.state;
    const { songs, playState, toggle, repeatType, page } = this.props;
    return (
      <>
        <Header
          playState={playState}
        ...
        {page === NOW_PLAYING_PAGE ? (
          <PlayingView
            ...
            playingSong={songs[playState.songId]}
            openSnackbar={msg =>
              this.setState({ snackBarOpen: true, snackMsg: msg })
            }
          />
        ) : (
          <MainView
